...
//#region : BuildSummer
function buildDetailedSommaire() {
  ...
  for (var i = 0; i < allTitles.length; i++) {
    ...
    // Vérifier le niveau du titre
    var level = parseInt(titreH2.id.split('.')[0]);

    // Réinitialiser la liste à puce courante si le niveau du titre est inférieur
    if (level <= currentLevel) {
      currentList = listeTdm;
      currentLevel = level;
    }

    // Masquer les titres qui n'ont pas d'ID se terminant par ".0"
    if (!/^\d+\.0$/.test(titreH2.id)) {

      // Vérifier si la liste imbriquée existe déjà
      var nestedList = currentList.lastElementChild;
      if (!nestedList || nestedList.nodeName !== 'UL') {
        // Créer une nouvelle liste à puce imbriquée
        nestedList = document.createElement('ul');
        nestedList.classList.add('hidden');
        currentList.appendChild(nestedList);
      }

      // Utiliser la liste imbriquée pour ajouter l'élément de liste
      nestedList.appendChild(elementListe);
    } else {
      // Réinitialiser la liste à puce courante
      currentList.appendChild(elementListe);
      currentList = elementListe;
      currentLevel = level;

      // Créer un bouton pour afficher/masquer la zone
      var button = document.createElement('button');
      button.classList.add("Bsommaire");
      button.textContent = '+';

      // Ajouter un gestionnaire d'événements au bouton
      button.addEventListener('click', function () {
        // Récupérer l'élément suivant (la liste à puces imbriquée)
        var ul = this.parentNode.nextSibling;
        console.log(ul);
        if (ul.nodeName === "UL") {
          // Basculer la classe "hidden" pour afficher/masquer la liste à puces imbriquée
